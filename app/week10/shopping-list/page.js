...
export default function Page() {
  ...
  async function loadItems() {
    try {
      if (user) {
        let loadItems = await getItems(user.uid);
        setItems(loadItems);
      }
    } catch (error) {
      console.error(error);
  ...
  function handleAddItem(newItem) {
    ...
    // });
    const newItemObj = addItem(user.uid, newItem);
    // const { id } = newItemObj;
    const id = newItemObj;
  ...
  return (
    <main className="bg-blue-100 pt-2 pb-2 h-full">
      {user ? (
        <div>
        ...
        <p className="bg-blue-100 h-screen m-2">
